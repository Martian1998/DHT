...
def put(fileName , current):
    ...
    if successorOfSuccessor.__len__()==0 or succOfFile.__len__()==0:
        return
...
def ping(current):
    ...
    while True:
        try:
            ...
            current.updateTableLeft(temp, predOfPredecessor)
            clientCon = socket.socket()
            clientCon.connect((current.pred[2],current.pred[1]))
            request = {
                'type' : 'updateTableLeft',
...
class dhtMain(QMainWindow):
    def __init__(self,current):
        super(dhtMain, self).__init__()
        self.free = True
        self.current = current
        loadUi('dhtgui.ui', self)
        self.setWindowTitle('DHT')
        self.portNo.setText(str(self.current.myHash))
        self.leaveBtn.clicked.connect(self.on_leaveBtn_clicked)
        self.downloadBtn.clicked.connect(self.on_downloadBtn_clicked)
        self.uploadBtn.clicked.connect(self.on_uploadBtn_clicked)
        self.showTab.clicked.connect(self.on_showTab_clicked)

    def on_showTab_clicked(self):
        from main import showTab
        print(self.current.fingerTable)
        print(self.current.succ)
        print(self.current.pred)
        try:
            if self.current.succ[0] == self.current.myHash and self.current.pred[0]==self.current.myHash:
                for i in self.current.fingerTable.keys():
                    self.current.fingerTable[i]=[ self.current.IP, self.current.port , self.current.myHash]
        except Exception as e:
            print(e)

        tabShower = showTab(self.current)
        tabShower.exec_()

    def on_leaveBtn_clicked(self):
        print(' leave clicked')
        leave(self.current)
        print('     ######Left#######   ')
        self.close()
    ...
    def on_uploadBtn_clicked(self):
        if self.free:
            self.free = False
            self.statusVal.setText('Uploading')
            print(self.uploadFile.text())
            put(self.uploadFile.text(),self.current)
            self.statusVal.setText('now idle')
            self.free = True
    def on_downloadBtn_clicked(self):
        if self.free:
            self.free = False
            self.statusVal.setText('Downloading')
            print(self.downloadFile.text())
            download(self.downloadFile.text(),self.current)
            self.statusVal.setText('now idle')
            self.free = True
